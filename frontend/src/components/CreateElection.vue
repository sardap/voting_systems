...
async function submit() {
  if (selected_election_type.value == ElectionType.FirstPastThePost) {
    error.value =
      'Are you stupid? I give you a bunch of good voting systems and you choose the worst one?'
    return
  }
  ...
  const response = await create_generic_election(
    selected_election_type.value,
    props.api_key,
    to_create.value
  )
  ...
  <div>
    ...
    <div v-else>
      ...
      <ElectionTypePicker
        ...
        :disabled="[ElectionType.QuotaPreferentialVicLabor2024]"
      />
      <br />
      <div v-if="error">
      ...
      <div :key="refresh_create">
        ...
        <PrefElectionCreate
          v-else-if="selected_election_type == ElectionType.PreferentialVoting"
          v-model="to_create"
        />
        <SvtCreate
          v-else-if="selected_election_type == ElectionType.SingleTransferableVote"
          v-model="to_create as StvCreateElection"
        />
        <BordaCountCreate
          v-else-if="selected_election_type == ElectionType.BordaCount"
          v-model="to_create"
        />
        <ApprovalCreate
          v-else-if="selected_election_type == ElectionType.Approval"
          v-model="to_create"
        />
        <StarCreate v-else-if="selected_election_type == ElectionType.Star" v-model="to_create" />
        <CumulativeCreate
          v-else-if="selected_election_type == ElectionType.Cumulative"
          v-model="to_create"
        />
        <AntiPluralityCreate
          v-else-if="selected_election_type == ElectionType.AntiPlurality"
          v-model="to_create"
        />
        <SinglePartyCreate
          v-else-if="selected_election_type == ElectionType.SingleParty"
          v-model="to_create"
        />
        <ThreeTwoOneCreate
          v-else-if="selected_election_type == ElectionType.ThreeTwoOne"
          v-model="to_create"
        />
        <CondorcetMethodCreate
          v-else-if="selected_election_type == ElectionType.CondorcetMethod"
          v-model="to_create"
        />
        <MajorityJudgmentCreate
          v-else-if="selected_election_type == ElectionType.MajorityJudgment"
          v-model="to_create"
        />
        <ScoreCreate v-else-if="selected_election_type == ElectionType.Score" v-model="to_create" />
        <UsualJudgmentCreate
          v-else-if="selected_election_type == ElectionType.UsualJudgment"
          v-model="to_create"
        />
        <SingleNonTransferableCreate
          v-else-if="selected_election_type == ElectionType.SingleNonTransferable"
          v-model="to_create"
        />
