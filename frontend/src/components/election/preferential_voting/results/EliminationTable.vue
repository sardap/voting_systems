...
onMounted(async () => {
  ...
  candidates_ordered.value = result.candidates.map((candidate_name, index) => {
    let eliminated_round: number | null = result.log.findIndex((round) =>
      round.eliminated.includes(index)
    )
...
function get_td_round_value(candidate: Candidate, round_number: number, round: LogEntry) {
  ...
  return (
    get_vote_percent(round.votes[candidate.original_index].votes) +
    '%' +
    ' (' +
    round.votes[candidate.original_index].votes +
    ')'
  )
}
...
<template>
  <table>
    <tr>
      <th>Name</th>
      <th v-for="round in election_result.log.length" :key="round">Round {{ round }}</th>
      <th>Rank</th>
    </tr>
    <tr v-for="[i, candidate] in candidates_ordered.entries()" :key="i">
      <td>{{ candidate.name }}</td>
      <td v-for="[round_number, round] in election_result.log.entries()" :key="round_number">
        {{ get_td_round_value(candidate, round_number + 1, round) }}
      </td>
