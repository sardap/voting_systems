...
<template>
  ...
  <MajorityJudgmentTally :options="result.options.map((name, i) => { return { option_index: i, name: name } })"
  ...
  <MajorityJudgmentTally
    ...
    :tally="result.runoff.modified_tally" :vote_count="result.vote_count" />
  <div v-if="result.score_result">
    <h2>Since another tie deciding winner based on score</h2>
    <ScoreCount :options="result.options" :counts="result.score_result.vote_tally" :winner="result.winner" />
  </div>
</template>
