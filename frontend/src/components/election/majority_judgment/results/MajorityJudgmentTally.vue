...
import { MJRating, type MajorityJudgmentTally, from_mj_rating_to_string } from '@/backend';
...
const ratings = [MJRating.VeryGood, MJRating.Good, MJRating.Acceptable, MJRating.Poor, MJRating.Terrible];
...
function make_dataset(rating: MJRating) {
    ...
    return { label: from_mj_rating_to_string(rating), data: dataset, backgroundColor: get_rating_color(rating) };
}

const bar_data = ref({
    labels: props.options.map((i) => i.name),
    datasets: [make_dataset(MJRating.VeryGood), make_dataset(MJRating.Good), make_dataset(MJRating.Acceptable), make_dataset(MJRating.Poor), make_dataset(MJRating.Terrible)],
});
...
<template>
    <div>
        ...
        <table>
            <tr>
                <th>Name</th>
                <th v-for="rating in ratings">{{ from_mj_rating_to_string(rating) }}</th>
                <th>Median</th>
            </tr>
