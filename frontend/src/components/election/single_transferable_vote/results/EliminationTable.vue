...
const quota = Math.max(1, Math.floor(props.election_result.votes.length / props.election_result.elected_candidates.length));
...
<template>
    <table>
        <tr>
            <th>Round</th>
            <th v-for="candidate in election.options">{{ candidate }}</th>
        </tr>
        <tr v-for="[round_idx, _] in election_result.rounds.entries()">
            <td class="first-col">{{ round_idx + 1 }}</td>
            <td v-for="[i, _] in election.options.entries()" :class="get_td_class(round_idx, i)">{{ get_td_value(round_idx,
                i) }}</td>
        </tr>
    </table>
</template>
