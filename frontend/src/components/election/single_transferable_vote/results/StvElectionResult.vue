...
import {
  type StvResult,
  get_single_transferable_vote_election_result,
  ElectionType,
...
async function refresh_results() {
  ...
  election.value = await get_generic_election(
    ElectionType.SingleTransferableVote,
    props.election_id
  )
  result.value = await get_single_transferable_vote_election_result(
    props.api_key,
    props.election_id,
    exclude_candidates.value
  )
...
function add_exclude_candidate() {
  ...
  exclude_candidate_options.value = exclude_candidate_options.value.filter(
    (i) => i != candidate_index
  )
    ...
    <div v-if="result && election">
      ...
      <div v-if="election">
        ...
        <select v-model="exclude_selected" @change="add_exclude_candidate">
          <option v-for="i in exclude_candidate_options" :value="i" :key="i">
            {{ election.options[i] }}
          </option>
        </select>
        <h4>Already selected exclude</h4>
        <ul>
          <li v-for="i in exclude_candidates" :key="i">{{ election.options[i] }}</li>
        </ul>
        <button @click="refresh_results">Refresh</button>
      </div>
      <div v-if="result.votes.length > 0">
        <h3>
          Quota Size: {{ Math.floor(result.votes.length / result.elected_candidates.length) }}
        </h3>
        <br />
        <h3>Winners</h3>
