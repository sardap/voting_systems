...
async function submit() {
  ...
  loading.value = true;
  const response = await submit_generic_vote(ElectionType.SingleTransferableVote, props.election.id, props.vote_token, votes);
...
<template>
  <div>
    ...
    <RankedChoice v-if="election" v-model="options" />
