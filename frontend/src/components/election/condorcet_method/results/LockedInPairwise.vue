...
function get_edges() {
    ...
    for (let i = 0; i < locked_in.length; i++) {
        for (let j = 0; j < locked_in[i].length; j++) {
            edges[`edge${edge_num + 1}`] = { source: `node${i + 1}`, target: `node${locked_in[i][j] + 1}` };
            edge_num++;
        }
    }
...
<template>
    <div>
        <table>
            ...
            <tr v-for="i in locked_in">
                <td>{{ get_name(i.from) }}</td>
                <td>
                    <div v-if="i.to.length > 0" v-html="i.to.map((i) => get_name(i)).join('<br/>')"></div>
                    <div v-else>
                        <p>None</p>
                    </div>
                </td>
        ...
        <br />
        <div>
            <v-network-graph class="graph" :nodes="nodes" :edges="edges" v-model:layouts="layouts" :configs="configs" />
        </div>
    </div>
</template>
