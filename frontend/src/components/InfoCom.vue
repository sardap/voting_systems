...
const info = [
    {
        ...
        summary: "Approval Voting is a voting system where each voter can approve of as many candidates as they like. There is no ranking or weighting of candidates. The candidate with the most approvals wins the election.",
    ...
    },
    {
        ...
        summary: "Borda Count is a voting system where candidates get points based on their ranking in each voter's preference list. The candidate with the highest total points wins.",
    ...
    },
    {
        ...
        summary: "Single Transferable Vote (STV) is a voting system where voters rank candidates, allowing multiple candidates to be elected in one election. Candidates reach a quota to be elected, and surplus votes or votes from eliminated candidates are redistributed until enough candidates are elected.",
        benefits: ["Provides proportional representation", "Encourages a diverse range of candidates", "Allows for more voter choice", "Supports electing multiple candidates"],
        issues: ["Can sometimes produce unexpected outcomes"],
    ...
    },
    {
        ...
        summary: "STAR Voting (Score Then Automatic Runoff) is a voting system where voters rate each candidate on a numerical scale (e.g., 0-5). The two candidates with the highest total scores proceed to an automatic runoff. In the runoff, the candidate who is preferred by more voters wins the election.",
    ...
    },
    {
        ...
        summary: "Cumulative voting is a system where voters have multiple votes to distribute among candidates, allowing them to give more support to their preferred candidates.",
        benefits: ["Encourages minority representation", "Allows voters to express varied levels of support for different candidates"],
    ...
    },
    {
        ...
        summary: "Anti-plurality voting, also known as plurality loser, disapproval voting or 'least-most' voting, is a voting system where voters can mark their disapproval for one candidate. The candidate with the most disapproval votes is eliminated, and the one with the least disapproval votes wins.",
        benefits: ["Discourages strategic voting", "Encourages voters to express their true preferences", "Favors candidates with broader appeal"],
        issues: ["Limited expressiveness", "Can be influenced by tactical voting", "Less known and studied compared to other voting methods"],
    ...
    },
    {
        ...
        issues: ["Lack of political diversity and choice for voters", "Imbalance of power", "Potential for corruption and authoritarian rule", "Limited accountability"],
    ...
    },
    {
        ...
        summary: "3-2-1 Voting is a voting system where voters rate each candidate as 'Good', 'OK', or 'Bad'. The three candidates with the highest 'Good' scores advance to the next round, where the candidate with the fewest 'Bad' scores is eliminated. In the final round, the candidate with the most 'OK' scores wins the election.",
        benefits: ["Simple to understand", "Encourages honest voting", "Reduces strategic voting", "Favors consensus candidates"],
        issues: ["May not always elect the most preferred candidate", "Less expressive than some other ranked systems"],
    ...
    },
    {
        ...
        summary: "Majority Judgment is a voting system where voters rate each candidate using words like 'Excellent', 'Good', 'OK', and so on. Candidates' ratings are compared, and the one with the best overall rating wins.",
        benefits: ["Encourages honest voting", "Reduces strategic voting", "Favors consensus candidates"],
    ...
    },
    {
        ...
        summary: "Score Voting, also known as range voting, is a voting system where voters give each candidate a score, usually within a predefined range (e.g., 0-10). The candidate with the highest average score wins the election.",
        benefits: ["Expressive", "Encourages honest voting", "Reduces strategic voting", "Favors consensus candidates"],
    ...
    },
    {
        ...
        summary: "Usual Judgment is a hypothetical variant of Majority Judgment, where voters rate each candidate using words like 'Excellent', 'Good', 'OK', and so on. However, unlike Majority Judgment, Usual Judgment compares candidates' average ratings instead of their median ratings. The candidate with the highest average rating wins the election.",
        benefits: ["Expressive", "Encourages honest voting", "Favors consensus candidates"],
        issues: ["More complex than some other systems", "Potential for ties", "Vulnerable to strategic manipulation"],
    ...
    },
    {
        ...
        summary: "Single Non-Transferable Vote (SNTV) is a voting method in which voters have one vote to cast in a multi-member district. The candidates with the most votes are elected. It's commonly used in multi-member district elections.",
        benefits: [
    ...
    },
    {
        ...
        summary: "First Past the Post (FPTP) is a voting method where the candidate with the most votes in each district wins. It's used in single-member district elections and is the simplest form of plurality/majority system.",
        benefits: [],
        issues: [
...
<template>
    <div>
        ...
        <ul>
            <li v-for="benefit in selected_info.benefits">{{ benefit }}</li>
        </ul>
        <p class="second-header">Issues</p>
        <ul>
            <li v-for="issue in selected_info.issues">{{ issue }}</li>
        </ul>
        <p class="second-header">Countries Used in</p>
        <ul v-if="selected_info.places_used_in.length > 0">
            <li v-for="county in selected_info.places_used_in">{{ county }}</li>
        </ul>
