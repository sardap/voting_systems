...
import { ElectionType, get_generic_election, type GenericElection, type GenericElectionResult, get_generic_election_result } from '@/backend';
  ...
  <div>
    ...
    <div>
      ...
      <div v-else-if="election">
        <ElectionResult v-if="api_key" :api_key="api_key" :election_type="election_type" :election="election" />
        <h2>Candidates</h2>
        ...
        <div v-if="result">
          ...
          <div v-if="result.vote_count > 0">
            <PrefElectionResult v-if="election_type == ElectionType.PreferentialVoting" :election="election"
              :result="result" />
            <StvElectionResult v-else-if="election_type == ElectionType.SingleTransferableVote && api_key"
              :election_id="election_id" :api_key="api_key" />
            <ScoreResult v-else-if="election_type == ElectionType.Score" :election="election" :result="result" />
            <MajorityJudgmentResult v-else-if="election_type == ElectionType.MajorityJudgment" :election="election"
              :result="result" />
            <ThreeTwoOneResult v-else-if="election_type == ElectionType.ThreeTwoOne" :election="election"
              :result="result" />
            <SinglePartyResult v-else-if="election_type == ElectionType.SingleParty" :election="election"
              :result="result" />
            <AntiPluralityResult v-else-if="election_type == ElectionType.AntiPlurality" :election="election"
              :result="result" />
            <CumulativeResult v-else-if="election_type == ElectionType.Cumulative" :election="election"
              :result="result" />
            <StarResult v-else-if="election_type == ElectionType.Star" :election="election" :result="result" />
            <BordaCountResult v-else-if="election_type == ElectionType.BordaCount" :election="election"
              :result="result" />
            <CondorcetMethodResult v-else-if="election_type == ElectionType.CondorcetMethod" :election="election"
              :result="result" />
            <UsualJudgmentResult v-else-if="election_type == ElectionType.UsualJudgment" :election="election"
              :result="result" />
            <ApprovalResult v-else-if="election_type == ElectionType.Approval" :election="election" :result="result" />
            <SingleNonTransferableResult v-else-if="election_type == ElectionType.SingleNonTransferable"
              :election="election" :result="result" />
          </div>
