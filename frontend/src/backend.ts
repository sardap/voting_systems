export enum ElectionType {
  ...
  QuotaPreferentialVicLabor2024 = 'Quota Preferential Vic Labor 2024'
}
...
export function election_type_to_path(election_type: ElectionType) {
  switch (election_type) {
    ...
    case ElectionType.QuotaPreferentialVicLabor2024:
      return 'quota_preferential_vic_labor_2024'
...
export async function change_extra(
  election_id: string,
  api_key: string,
  new_public?: boolean,
  new_voting_lock?: boolean
) {
  const params = new URLSearchParams({ api_key: api_key })
...
export async function get_generic_election_result(
...
export async function submit_generic_vote(
  election_type: ElectionType,
  election_id: string,
...
export async function create_generic_election(
  election_type: ElectionType,
  api_key: string,
  arg: GenericElectionCreate
) {
  const params = new URLSearchParams({ api_key: api_key })
...
async function get_result<T>(
  ...
  const response = await fetch(
    `/api/v1/${election_type_to_path(election_type)}/${election_id}/get_result?${params.toString()}`
  )
...
export async function get_preferential_voting_result(
  api_key: string,
  election_id: string
): Promise<PrefElectionResult> {
...
export async function get_single_transferable_vote_election_result(
  api_key: string,
  election_id: string,
  eliminated_candidates: number[]
): Promise<StvResult> {
  ...
  const response = await fetch(
    `/api/v1/single_transferable_vote/${election_id}/get_result?` + params.toString()
  )
  return await response.json()
}
...
export async function get_borda_count_election_result(
  api_key: string,
  election_id: string
): Promise<BordaCountResult> {
...
export async function get_approval_election_result(
  api_key: string,
  election_id: string
...
export async function get_star_election_result(
  api_key: string,
  election_id: string
): Promise<StarResult> {
...
export async function get_cumulative_result(
  api_key: string,
  election_id: string
...
export async function get_anti_plurality_result(
  api_key: string,
  election_id: string
): Promise<AntiPluralityResult> {
  return await get_result<AntiPluralityResult>(ElectionType.AntiPlurality, election_id, api_key)
}

export async function submit_anti_plurality_vote(
  election_id: string,
  vote_token: string | undefined,
  vote: number
) {
  ...
  const response = await fetch(
    `/api/v1/${election_type_to_path(ElectionType.AntiPlurality)}/${election_id}/new_vote?` +
      params.toString(),
    {
      method: 'POST',
      headers: {
...
export async function submit_single_party_vote(
  election_id: string,
  vote_token: string | undefined,
  voted: boolean
) {
  ...
  const response = await fetch(
    `/api/v1/${election_type_to_path(ElectionType.SingleParty)}/${election_id}/new_vote?` +
      params.toString(),
    {
      method: 'POST',
      headers: {
...
export async function get_single_party_result(
  api_key: string,
  election_id: string
): Promise<SinglePartyResult> {
...
export async function get_three_two_one_result(
  api_key: string,
  election_id: string
): Promise<ThreeTwoOneResult> {
...
export async function get_condorcet_method_result(
  api_key: string,
  election_id: string
): Promise<CondorcetMethodResult> {
...
export async function get_majority_judgment_result(
  api_key: string,
  election_id: string
): Promise<MajorityJudgmentResult> {
  return await get_result<MajorityJudgmentResult>(
...
export interface QuotaPreferentialVicLabor2024Candidate {
  name: string
  is_female: Boolean
}

export interface QuotaPreferentialVicLabor2024CreateElection {
  title: string
...
export interface QuotaPreferentialVicLabor2024Paper {
  vote: QuotaPreferentialVicLabor2024Vote
  value: number
}

export interface QuotaPreferentialVicLabor2024CountSheetEntry {
  option_index: number
  papers: QuotaPreferentialVicLabor2024Paper[]
  separated_papers: string[]
  transfer_value: number
  remainders: number
}

export interface QuotaPreferentialVicLabor2024CountSheet {
  entries: Record<number, QuotaPreferentialVicLabor2024CountSheetEntry>
  exhausted_papers: QuotaPreferentialVicLabor2024Paper[]
}

export interface QuotaPreferentialVicLabor2024CountSheetCollection {
  count_sheets: QuotaPreferentialVicLabor2024CountSheet[]
  aa_count_sheets: QuotaPreferentialVicLabor2024CountSheet[]
}

export interface QuotaPreferentialVicLabor2024DecisionLog {
  decisions: string[]
}

export interface QuotaPreferentialVicLabor2024ElectionResult {
  elected_candidates: number[]
  quota: number
  count_sheet: QuotaPreferentialVicLabor2024CountSheetCollection
  decision_log: QuotaPreferentialVicLabor2024DecisionLog
}

export interface VoteBundle<T> {
  count: number
  vote: T
}

export interface QuotaPreferentialVicLabor2024Result extends GenericElectionResult {
  candidates: string[]
  votes: VoteBundle<number[]>[]
  result: QuotaPreferentialVicLabor2024ElectionResult
}

export interface QuotaPreferentialVicLabor2024Vote {
  created_by: string
  votes: number[]
}

export interface TestElectionBase {
  election: any
  bundles: any
}

export async function test_election(election_type: ElectionType, body: TestElectionBase) {
  const response = await fetch(`/api/v1/${election_type_to_path(election_type)}/test`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
