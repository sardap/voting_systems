...
use voting_systems::single_non_transferable_vote::SNTVVote;
...
use crate::{
    ...
    single_non_transferable_vote::{
        ...
        SNTVCreateElection, SNTVElectionModeled, SNTVVoteModeled,
    },
};
...
fn make_vote(request: &SNTVVoteRequest, user_id: &uuid::Uuid) -> SNTVVoteModeled {
    SNTVVoteModeled(SNTVVote {
...
fn vote_validation(
    ...
    election: &SNTVElectionModeled,
) -> Result<(), crate::routes::api::common::NewVoteError> {
    use crate::routes::api::common::NewVoteError;
    if election.sntv_election.options.len() != request.votes.len() {
